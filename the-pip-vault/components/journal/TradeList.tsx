      ...
      <div className="flex flex-col sm:flex-row p-4 pl-6 gap-6">
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center">
            <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-pip-muted text-xs flex items-center gap-1 ml-auto sm:ml-0">
                    <Calendar size={12} />
                    {new Date(trade.date).toLocaleDateString('nl-NL', { day: '2-digit', month: 'short', hour: '2-digit', minute:'2-digit' })}
                </span>
            </div>
        ...
        <div className="flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-4 sm:pl-6 sm:border-l border-pip-border min-w-30">
            <div className="text-right">
                <div className={`text-2xl font-bold ${textColor}`}>{trade.pnl > 0 ? '+' : ''}{trade.pnl}</div>
                <div className="text-xs text-pip-muted uppercase tracking-wider font-medium">Pips</div>
            </div>
            {trade.rrRatio && (
